...
int q_submit_job(job_queue_t *q, int t, int id) {
    ...
    if (r < 0)
    {
        fprintf(stderr, "Error: computer %d tries to submit a job after the max number of jobs have been submitted.", id);
        exit(1);
    } else if (r == 0) {
        fprintf(stderr, "Error: computer %d tries to submit a job when the queue is full.", id);
        exit(1);
    }
...
int q_fetch_job(job_queue_t *q, int id) {
    ...
    if (r < 0) {
        fprintf(stderr, "Error: printer %d tries to get a job after the max number of jobs have been started.", id);
        exit(1);
    } else if (r == 0) {
        fprintf(stderr, "Error: printer %d tries to start a job while no job is pending.", id);
        exit(1);
    }
